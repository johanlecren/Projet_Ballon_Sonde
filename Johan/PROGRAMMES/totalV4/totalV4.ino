...
TaskHandle_t Tasksensor;  //instanciation de la tâche sensor
TaskHandle_t Tasksigfox;  //instanciation de la tâche sigfox
TaskHandle_t Taskmsd;  //instanciation de la tâche msd

QueueHandle_t queueSigfox;  //instanciation de la file sigfox
QueueHandle_t queueMSD;  //instanciation de la file msd
QueueHandle_t queueEnvoiBouton; //instanciation de la file EnvoiBouton

sauvegardeMSD carteSd;  // A mettre en global pour que le handler web puisse y accéder
...
void setup() {
  ...
  xTaskCreatePinnedToCore(
    ...
    1);          /* pin de la tâche pour le coeur 1 */
  ...
  xTaskCreatePinnedToCore(
    ...
    1);          /* pin de la tâche pour le coeur 0 */
...
void  msdTask( void * pvParameters ) {
  ...
  if (erreurCarte == false) {
    if (cartesd.fichierPresent(SD, "/telemesures.txt") == false) { //vérification de la présence du fichier "telemesures.txt"
      erreurWrite = cartesd.writeFile(SD, "/telemesures.txt", "Date;Heure;Latitude;Longitude;Altitude;Pression;Humidité;Température;Impulsions\n"); // ecriture des données en tete dans le fichier "telemesures.txt"
  ...
  while (1) {
    ...
    if (erreurCarte == false) {
      erreurAppend = cartesd.appendFile(SD, "/telemesures.txt", ligneCsv);
